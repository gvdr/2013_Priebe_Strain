...
Within the supplied set of files, \texttt{rsos.cls} need to be copied into a directory
where tex looks for input files. The other files need to be kept as a reference while preparing your manuscript.
Please use pre-defined commands from \texttt{Author\_tex.tex} for title, authors, address, abstract, keywords, body etc.

\section{How to start using rsos.cls}
Before you type anything that actually appears in the paper you need to include a \verb+\documentclass{rsos}+ command
...
\noindent Note: For the acknowledgement section use the command \verb+\ack{}+.

\vskip2.5pc

\noindent The rsos file is intended as a guide only. Article lengths based
