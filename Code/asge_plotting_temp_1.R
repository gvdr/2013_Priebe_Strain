...
strain_data <- function(Graph,Rank){
  ...
  #pagerank <- page.rank(Graph)$vector #THERE A FUFFING BUG IN THE ARPACK LAPACK
  ...
  dataset <- data.frame(strain,eigenvector_centrality,
                        #pagerank,
                        closeness,betweenness,out_degree,in_degree,degree)
...
strain_graph_plot(Name_S_D_s,G_S_D_s)
...
RS_S_D_s_4 = regsubsets(strain ~ . , data = DF_S_D_s_4, nbest = 1)
plot(RS_S_D_s_4, scale = "adjr2", main="Serengeti (de Visser) Strain linear regression, rank 4")

plot(strain ~ degree, data=DF_S_B_4, main="Serengeti (Baskerville), rank 4")
...
#pdf plot a histogram of the strain
strain_title = paste("Strain_Serengeti_Baskerville +
                     ",2,sep="_")
strain_hist = ggplot(DF_S_B_2, aes(x=strain))
strain_hist + geom_histogram(binwidth = 0.01)
ggsave(file = paste(strain_title,".pdf",sep=""))


strains_data_D_4 = strain_data(Graph_D,4)
with(DF_S_B_4, invTranPlot(strain, degree, main="Serengeti (Baskerville) rank 4 embedding strain"))

par(mfrow=c(1,4))
plot(strain ~ degree, data = DF_S_B_2)
plot(strain ~ out_degree, data = DF_S_B_2)
plot(strain ~ betweenness, data = DF_S_B_2)
plot(strain ~ closeness, data = DF_S_B_2)