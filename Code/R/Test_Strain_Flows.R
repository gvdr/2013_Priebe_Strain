...
compute_strain(adj,RANK,mode=MODE) %>%
  setNames(row.names(adj)) -> topo_strain

mclapply( 1:NSIM,
          function(x){
            comparison_flow(adj,Rank=RANK,Mode=MODE,Mean_Noise,Var_Noise)
          },
...
sapply(1:NSIM,
       function(x){
         ot[[x]] %>%
           .[names(topo_strain)] %>%
         {lm(.~ topo_strain)} -> model
        c(model %>% tidy %$% p.value %>% .[2],
...
freq_higher %>% colMeans %>%
  ...
  ggplot(aes(x=reorder(species_names, -topo_strain[species_names]),y=Frequency)) +
  geom_point() +
  ylim(c(0,1)) +
  ...
  geom_hline(yintercept = Higher_Window/length(topo_strain),color="red",linetype = 3) +
  theme_minimal() +
...
paste0(TheChosenNetwork," ",MODE,", significants = ", freq.p %>% percent,
  ...
  print
paste0("../Plots/Weighted/",TheChosenNetwork,"_bp_",MODE,".pdf") %>%
